...
class _MyBottomNavigationBarState extends State<MyBottomNavigationBar> with SingleTickerProviderStateMixin {
        ...
        child: (_currentIndex == index)
            ? Wrap(
                ...
                children: [
                  ...
                  if (widget.labelDirection == Axis.horizontal)
                    MySpacing.width(showActiveLabel ? widget.labelSpacing : 0)
                  else
                    MySpacing.height(showActiveLabel ? widget.labelSpacing : 0),
                  if (showActiveLabel)
                    Text(
                      item.title!,
                      style: activeTitleStyle ??
                          ...
                          MyTextStyle.labelSmall(
                            color: activeTitleColor ?? item.activeTitleColor ?? theme.primaryColor,
                ...
                children: [
                  item.icon ??
                      Icon(
                        ...
                        color: iconColor ?? item.iconColor ?? theme.colorScheme.onSurface,
                      ),
                  if (widget.labelDirection == Axis.horizontal)
                  ...
                  if (showLabel)
                    Text(
                      ...
                      style: titleStyle ??
                          item.titleStyle ??
                          MyTextStyle.labelSmall(
                            color: titleColor ?? item.titleColor ?? theme.colorScheme.onSurface,
      ...
      return (_currentIndex == index)
          ...
          : item.icon ??
              Icon(
                ...
                color: iconColor ?? item.iconColor ?? theme.colorScheme.onSurface.withAlpha(150),
              );
    }
  }
  ...
  List<Widget> getListOfItems() {
    ...
    double singleWidth = (MediaQuery.of(context).size.width - 50) /
        (itemList!.length + (widget.showLabel ? 0 : (showActiveLabel ? 0.5 : 0)));

    for (int i = 0; i < itemList!.length; i++) {
      double containerWidth = widget.showLabel
          ? singleWidth
          : (showActiveLabel ? (i == _currentIndex ? singleWidth * 1.5 : singleWidth) : singleWidth);
      list.add(
        SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    myBottomNavigationBarType = widget.myBottomNavigationBarType ?? MyBottomNavigationBarType.normal;
    showLabel = widget.showLabel;
    showActiveLabel = widget.showActiveLabel ?? true;
    activeContainerColor = widget.activeContainerColor ?? theme.primaryColor.withAlpha(100);
