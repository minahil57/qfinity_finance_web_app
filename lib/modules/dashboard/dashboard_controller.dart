...
class DashboardController extends BaseController {
  ...
  final List<DateTimeFilterDropdown> dateTimeFilterItems = [
  ...
  List<VoucherCountModel> vouchersCount = [];

  DateTimeFilterDropdown selectedDateFilter = DateTimeFilterDropdown.last30Days();

  void onSelectedDateFilter(DateTimeFilterDropdown filter) {
    selectedDateFilter = filter;
    getVouchersCount();
    update();
  }

  Future<void> getVouchersCount() async {
    try {
      var response = await ApiProvider.generalFinance.getDashboard(
        data: {
          'fromDate': selectedDateFilter.startDate.startOfDay,
          'toDate': DateTime.now().endOfDay,
          'financialYear': Get.find<StarterController>().selectedFinancialYearId,
        },
      );

      if (response.statusCode == 200) {
        vouchersCount = (response.data['data']['vouchers'] as List).map((e) => VoucherCountModel.fromJson(e)).toList();
