...
class CreatePettyCashVoucherController extends BaseVoucherController<CreatePettyCashVoucherItemModel>
    with
        CostCentersMixin,
        AttachmentMixin,
        ExportMixin,
        CurrencySelectionMixin,
        TaxCodeMixin<CreatePettyCashVoucherItemModel>,
        NarrationMixin<CreatePettyCashVoucherItemModel> {
  ...
  CreatePettyCashVoucherItemModel? toBeEditedItem;

  @override
  Future<void> onInit() async {
    super.onInit();
    AppLogger.d('CreatePettyCashVoucher._initData');
  ...
  void _onEditItemIconPressed(CreatePettyCashVoucherItemModel item) {
    toBeEditedItem = item;
    _openModalForm();
  }
  ...
  void _openModalForm() {
    Modals.formModal(
      form: const CreatePettyCashVoucherItemForm(),
    );
  }
  ...
  Future<void> saveVoucher() async {
    if (getValidations()) {
      try {
        EasyLoading.show(
          status: 'Creating Petty Cash Voucher...',
          maskType: EasyLoadingMaskType.black,
        );
        await uploadImages();
        var response = await ApiProvider.pettyCashVouchers.createPettyCadhVoucher(data: buildVoucherData());

        if (response.statusCode == 200) {
          voucherNo = response.data['data']['voucherNo'];
          Snackbars.success(
            response.data['data']['voucherNo'] + ' created successfully.',
          );
  ...
  DataRow _buildVoucherItemTableSingleMainRow({
    required CreatePettyCashVoucherItemModel item,
  }) {
    return DataRow(
      cells: [
        ...[
  ...
  void _deleteVoucherItem(CreatePettyCashVoucherItemModel item) {
  ...
  Map<String, dynamic> buildVoucherData() {
    ...
    CreatePettyCashVoucherModel voucherModel = CreatePettyCashVoucherModel(
  ...
  @override
  List<CreatePettyCashVoucherItemModel> buildVoucherItemsBeforeSend() {
    List<CreatePettyCashVoucherItemModel> items = [];
    for (final item in voucherItems) {
      items.add(
        CreatePettyCashVoucherItemModel(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (fileUrl != null) {
          ...
          Get.to(
            () => const PrintPage<CreatePettyCashVoucherController>(
